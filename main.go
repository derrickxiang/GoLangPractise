...
func main() {
	...
	http.HandleFunc("/singapore/", func(w http.ResponseWriter, r *http.Request){
		date_time := strings.SplitN(r.URL.Path, "/", 3)[2]

		fmt.Println(date_time)

		//fmt.Println(queryAPI(date_time))
		
		w.Header().Set("Content-Type", "application/json; charset=utf-8")
		json.NewEncoder(w).Encode(map[string]interface{}{
			"dateTime": date_time,
			"Weather": queryAPI(date_time),
		})
...
func queryAPI(date_time string) string {

	url := "https://api.data.gov.sg/v1/environment/air-temperature?date_time=" + date_time
